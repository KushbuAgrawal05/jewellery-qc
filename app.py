    ...
    <style>
      ...
      html, body, [data-testid="stAppViewContainer"], [data-testid="stAppViewContainer"] > .main {
        ...
        background-image:
          ...
          radial-gradient(circle at 12% 20%, rgba(184,137,43,.55) 0 1.6px, transparent 1.6px),
          radial-gradient(circle at 68% 8%, rgba(231,184,189,.55) 0 1.3px, transparent 1.3px),
          radial-gradient(circle at 40% 55%, rgba(184,137,43,.4) 0 1.1px, transparent 1.1px),
          radial-gradient(circle at 88% 42%, rgba(184,137,43,.5) 0 1.8px, transparent 1.8px),
          radial-gradient(circle at 22% 78%, rgba(231,184,189,.5) 0 1.4px, transparent 1.4px),
          radial-gradient(circle at 78% 82%, rgba(184,137,43,.45) 0 1.2px, transparent 1.2px),
          radial-gradient(circle at 55% 30%, rgba(184,137,43,.3) 0 .9px, transparent .9px),
          radial-gradient(circle at 5% 50%, rgba(184,137,43,.35) 0 1px, transparent 1px);
        background-repeat: no-repeat, repeat, repeat, repeat, repeat, repeat, repeat, repeat, repeat;
        background-size: cover, 150px 150px, 150px 150px, 150px 150px, 150px 150px, 150px 150px, 150px 150px, 150px 150px, 150px 150px;
        color: var(--ink);
      ...
      h2 {font-size: 2.6rem;} h3 {font-size: 2.05rem;}
      ...
      .hero-mark img {display:block; height: 70px; width: auto;}
      .hero-title {position: relative; display: inline-block;}
      .hero h1 {
        font-size: 3.8rem; font-weight: 700; margin: 0; line-height: 1.05;
        ...
        filter: drop-shadow(0 2px 10px rgba(184,137,43,.25));
      }
      .hero p {color: var(--ink-soft); margin: .6rem 0 0; font-size: 1.3rem; font-weight: 400;}
      .sparkle {
      ...
      /* ---------- Full-page twinkling sparkle overlay ---------- */
      ...
      .verdict {
        ...
        background: var(--bg); margin-bottom: 1.1rem; border-radius: 18px;
        box-shadow: 0 6px 20px rgba(43,35,23,.07);
      }
      .verdict .label {font-family: 'Cormorant Garamond', serif; font-size: 2.5rem; font-weight: 700; line-height: 1.15; color: var(--ink);}
      .verdict .sub {opacity: .75; font-size: 1.05rem; margin-top: .35rem; letter-spacing: .01em;}
      ...
      .step .icon-badge {
        ...
        background: linear-gradient(135deg, rgba(184,137,43,.14), rgba(231,184,189,.22));
      }
      .step .icon {color: var(--gold); display:block;}
      .step .num {font-family: 'Cormorant Garamond', serif; color: var(--gold); font-size: 2rem; font-weight: 700; display:block; margin-bottom: .25rem;}
      .step b {font-family: 'Cormorant Garamond', serif; font-size: 1.5rem; font-weight: 700; color: var(--ink); display:block; margin-bottom:.4rem;}
      .step span.body {color: var(--ink-soft); font-size: 1.05rem; line-height: 1.55;}
      ...
      [data-testid="stFileUploaderDropzone"] {
        background: var(--pearl-panel); border: 1px dashed var(--hairline); border-radius: 18px;
      }
      ...
      [data-testid="stExpander"] { border: 1px solid var(--hairline); border-radius: 16px; background: rgba(184,137,43,.03); }
...
_SPARKLE_SPOTS = [
    (6, 12, ".9rem", "var(--gold-light)", .0), (14, 82, "1.1rem", "var(--rose)", .6),
    (22, 45, ".7rem", "var(--gold)", 1.3), (30, 68, ".8rem", "var(--gold-light)", 2.1),
    (40, 8, "1rem", "var(--rose)", .9), (48, 90, ".7rem", "var(--gold)", 1.7),
    (58, 25, ".9rem", "var(--gold-light)", 2.6), (66, 55, "1.1rem", "var(--rose)", .4),
    (74, 78, ".8rem", "var(--gold)", 1.1), (82, 15, ".7rem", "var(--gold-light)", 2.3),
    (88, 60, "1rem", "var(--rose)", 1.9), (95, 35, ".8rem", "var(--gold)", .7),
]
_sparks = "".join(
    f'<span style="top:{t}%; left:{l}%; font-size:{s}; color:{c}; animation-delay:{d}s;">'
    f'{"✦" if i % 2 == 0 else "✧"}</span>'
    for i, (t, l, s, c, d) in enumerate(_SPARKLE_SPOTS)
)
st.markdown(f'<div class="sparkle-field">{_sparks}</div>', unsafe_allow_html=True)
...
st.markdown(
    ...
    '<span class="sparkle" style="top:40%; left:-30px; animation-delay:.9s; font-size:.65rem;">✧</span>'
    '<span class="sparkle" style="bottom:-6px; right:20%; animation-delay:1.9s; font-size:.9rem;">✧</span>'
    "</div>"
...
if upload is None:
    st.markdown(
        ...
        <div class="steps">
          <div class="step">
            <span class="icon-badge"><span class="icon"><svg viewBox="0 0 48 48" width="28" height="28" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M24 6v22M16 16l8-8 8 8"/><path d="M8 30v8a2 2 0 0 0 2 2h28a2 2 0 0 0 2-2v-8"/></svg></span></span>
            <span class="num">I</span><b>Upload</b><span class="body">A photo of the piece, or the area you're checking.</span>
          </div>
          <div class="step">
            <span class="icon-badge"><span class="icon"><svg viewBox="0 0 48 48" width="28" height="28" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M24 5l15 6v11c0 10-6.5 17-15 21C15.5 39 9 32 9 22V11z"/><path d="M17 24l5 5 10-11"/></svg></span></span>
            <span class="num">II</span><b>Verdict</b><span class="body">Defect / no defect, plus the top suspected defect types.</span>
          </div>
          <div class="step">
            <span class="icon-badge"><span class="icon"><svg viewBox="0 0 48 48" width="28" height="28" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><circle cx="21" cy="21" r="13"/><path d="M30.5 30.5 41 41"/></svg></span></span>
